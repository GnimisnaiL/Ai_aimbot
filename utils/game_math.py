...
def get_approx_distance(xywhd, model_x, model_y):
    ...
    fix_aspect = max(0.20, min(0.5, aspect))
...
#根据距离获取锁头位置
def get_headshot_position_by_dist(mode, dist, Py):
    #print(dist)
    if mode['headshot']:
        lower = mode['headshot_dist']
        upper = mode['headshot_dist']+3
        if dist <= lower:
            return mode['headshot_pos']
        elif dist >= upper:
            return Py
        else:
            # 线性插值
            ratio = (dist - lower) / (upper - lower)  
            return mode['headshot_pos'] + ratio * (Py - mode['headshot_pos'])
    else:
        return Py

#根据目标体型或者距离 进行速度限制或者平滑
def smooth_poccess(final_x, final_y, target, Sx, Sy, delay_factor, dist_factor, smooth):
