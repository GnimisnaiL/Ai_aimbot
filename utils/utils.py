...
class BaseEngine(object):
    ...
    def infer(self, img):
        ...
        #cpu2gpu_time = (time.time() - start_time) * 1000  
        #print(f"cpu2gpu: {cpu2gpu_time:.2f} ms")

        self.context.execute_v2(self.allocations)

        #infer_time = (time.time() - start_time) * 1000 - cpu2gpu_time 
        #print(f"infer: {infer_time:.2f} ms")

        for o in range(len(outputs)):
            common.memcpy_device_to_host(outputs[o], self.outputs[o]['allocation'])

        #gpu2cpu_time = (time.time() - start_time) * 1000 -cpu2gpu_time -infer_time  
        #print(f"gpu2cpu: {gpu2cpu_time:.2f} ms")
        return outputs
